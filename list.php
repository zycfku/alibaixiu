...
<body>
  ...
  <div class="entry">
          <div class="head">
            ...
            <a href="detail.php?slug={{v.slug}}">{{v.title}}</a>
          </div>
          <div class="main">
            ...
            <p class="extra">
              ...
              <span class="comment">评论({{v.comments}})</span>
              <a href="javascript:;" class="like">
                <i class="fa fa-thumbs-up"></i>
                <span>赞({{v.likes}})</span>
              </a>
  ...
  <script type="text/template" id="cidMoreData">
  {{each list as v k}}
  <div class="entry">
          <div class="head">
            <span class="sort">{{v.name}}</span>
  ...
  $.ajax({
    ...
    success: function(res) {
        if(res.code==1) {
          console.log(res);
          var str = template('listData',{list: res.data});
          $('.new').append(str);
          var divMore = `<div class="more">
  ...
  var count = 1; // 用于记录点击的次数 
    $('.new').on('click','.btn',function(){
       count++;
       // 告诉服务器我点击了几次 需要发送ajax给服务器
       $.ajax({
         type:'post',
         ...
         success:function(res){
          if(res.code == 1){
            var str = template('cidMoreData',{list:res.data});
            $('.more').before(str);
            
            var totalCount = Math.ceil(res.oneData / 10);
            
            // 如果count这个变量的值 等于 了totalCount了 
            if(count == totalCount){
              $('.more').hide();
            }
          }
